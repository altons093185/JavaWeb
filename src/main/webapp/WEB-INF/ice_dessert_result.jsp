				...
				<tbody>
					<c:forEach varStatus="i" var="iceOrder" items="${ iceOrders }">
						<tr>
							...
							<td><fmt:formatNumber value="${ iceOrder.totalPrice }"
									type="currency" maxFractionDigits="0" /></td>
							<td title="按我一下刪除" style="cursor: pointer;">
								<!-- <button class="delete-btn" data-index="${ i.index }">❌</button> -->
								<form method="POST" action="/JavaWeb/deleteIceOrder"
									style="display: inline;">
									<button type="submit" class="pure-button" name="index" value="${ i.index }">❌</button>
								</form>
							</td>
						</tr>
						<!-- 累加總價 -->
						<c:set var="totalPriceSum"
							value="${ totalPriceSum + iceOrder.totalPrice }" />
					</c:forEach>
				</tbody>
				<tfoot>
					<tr style="background-color: #DDDDDD">
						<td colspan="3" style="text-align: right;">總金額</td>
						<td><fmt:formatNumber value="${ totalPriceSum }"
								type="currency" maxFractionDigits="0" /></td>
					</tr>
				</tfoot>
			</table>
